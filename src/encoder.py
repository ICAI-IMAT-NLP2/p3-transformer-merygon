...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Apply layer normalization, then multi-head attention with residual connection
        hidden_state = self.layer_norm_1(x)
        attention_output = self.attention(hidden_state, hidden_state, hidden_state, mask)
        x = x + attention_output

        # Apply layer normalization, then feed-forward network with residual connection
